...
int FastServer::OnProcessRequest(IOBuf& frame, void* arg) {
    ...
    uint32_t payload_len = total_len - 8 - meta_len - attachment_size;
    ...
    // Parse payload.
    {
        ...
        if (!request->ParseFromZeroCopyStream(&zcis)) {
            LOG_ERR("OnProcessRequest: failed to parse request");
            return -1;
        }
    }
...
void FastServer::ReturnRPCResponse(CallBackArgs args) {
    ...
    uint32_t payload_len    = args.response->ByteSizeLong();
    uint32_t total_len      = 12 + payload_len + attachment_len;

    // Build frame: [total_len(BE)][rpc_id(BE)][attachment_size(BE)][payload][attachment]
    IOBuf frame;
    uint32_t be;

    be = htonl(total_len);
    frame.append(&be, 4);
    be = htonl(args.rpc_id);
    frame.append(&be, 4);
    be = htonl(attachment_len);
    frame.append(&be, 4);
